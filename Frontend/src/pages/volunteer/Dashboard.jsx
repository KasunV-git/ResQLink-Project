...
export default function Dashboard({ user, onToggleAvailability, activeAssignments = [], completedAssignments = [], alerts = [], onTabChange }) {
  ...
  return (
    <div className="flex flex-col w-full gap-4 md:gap-5" data-name="VolunteerDashboard">

      {/* Title */}
      <div className="anim-fade-in-up">
        <h1 className="text-2xl md:text-[26px] font-bold text-slate-900 dark:text-white mb-1">{t("dashboard.title") || "Volunteer Dashboard"}</h1>
        <p className="text-sm md:text-[14px] text-slate-500 dark:text-slate-400">
          {t("dashboard.subtitle", { name: user?.name || "Volunteer" }) || `Welcome, ${user?.name || "Volunteer"}! Manage your assignments and availability.`}
        </p>
      </div>
      ...
      <div className={`anim-fade-in-up d-100 hover-card bg-white dark:bg-slate-900 rounded-2xl px-5 py-4 md:px-6 border transition-all shadow-xs ${
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h2 className="text-[17px] font-semibold text-slate-900 dark:text-white mb-1">{t("dashboard.availabilityStatus") || "Availability Status"}</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {isAvailable ? (t("dashboard.availableDesc") || "You are currently available for assignments") : (t("dashboard.unavailableDesc") || "You are currently unavailable for assignments")}
            </p>
          </div>
          <div className="flex items-center gap-3 flex-shrink-0">
            <span className={`text-sm font-semibold transition-colors duration-300 ${
              ...
              {isAvailable ? (t("dashboard.available") || "Available") : (t("dashboard.unavailable") || "Unavailable")}
            </span>
            {/* Toggle switch */}
            <button
              onClick={onToggleAvailability}
              type="button"
              className={`relative cursor-pointer flex-shrink-0 rounded-full transition-colors duration-300 w-12 h-6.5 border-none p-0 ${
        ...
        <div className="anim-fade-in-up d-150 bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 shadow-xs flex flex-col min-h-[300px]">
          <div className="flex justify-between items-center pb-3 border-b border-slate-100 dark:border-slate-800 mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[15px] font-semibold text-slate-900 dark:text-white">{t("dashboard.activeAssignments") || "Active Assignments"}</span>
            </div>
          ...
          <div className="flex-1 overflow-y-auto flex flex-col gap-2.5">
            {activeAssignments.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 gap-2 py-8">
                ...
                <span className="text-sm">{t("dashboard.noActiveAssignments") || "No active assignments"}</span>
              </div>
        ...
        <div className="anim-fade-in-up d-200 bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 shadow-xs flex flex-col min-h-[300px]">
          <div className="flex justify-between items-center pb-3 border-b border-slate-100 dark:border-slate-800 mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[15px] font-semibold text-slate-900 dark:text-white">{t("dashboard.recentAlerts") || "Recent Alerts"}</span>
            </div>
      ...
      <div className="grid grid-cols-3 gap-3 md:gap-4">
        {[
          { labelKey: "dashboard.totalAssignments", fallback: "Total Assignments", value: totalCount,    color: "text-[#15803d] dark:text-emerald-400" },
          { labelKey: "dashboard.activeTasks",      fallback: "Active Tasks",      value: activeCount,   color: "text-amber-600 dark:text-amber-400" },
          { labelKey: "dashboard.completed",        fallback: "Completed",         value: completedCount, color: "text-emerald-600 dark:text-emerald-400" },
        ].map(({ labelKey, fallback, value, color }, i) => (
