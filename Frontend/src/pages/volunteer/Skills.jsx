  ...
  return (
    <div className="w-full flex flex-col gap-4 md:gap-6" data-name="SkillsPage">
      ...
      <div className="flex flex-col gap-1">
        <h1 className="font-semibold text-3xl text-slate-900 tracking-tight">{t("skills.title")}</h1>
        <p className="text-slate-500 text-base">{t("skills.subtitle")}</p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 md:p-6 shadow-sm flex flex-col gap-4">
        <div className="flex flex-wrap justify-between items-center gap-2 pb-2">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-base text-slate-900">
              {t("skills.yourSkills", { count: currentSkills.length })}
            </h2>
          </div>
          <button
        ...
        <div className="flex flex-wrap gap-2.5 min-h-[60px] p-2 bg-slate-50/30 border border-dashed border-slate-200 rounded-lg items-center">
          {currentSkills.length === 0 ? (
            <p className="text-slate-400 text-sm pl-2">{t("skills.noSkills")}</p>
          ) : (
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 md:p-6 shadow-sm flex flex-col gap-4">
        <div className="flex flex-col gap-1 pb-2">
          <h3 className="font-semibold text-base text-slate-900">{t("skills.suggestedSkills")}</h3>
          <p className="text-slate-400 text-xs">{t("skills.suggestedDesc")}</p>
        </div>

        <div className="flex flex-wrap gap-2.5">
          {suggestedSkills.length === 0 ? (
            <p className="text-slate-400 text-sm">{t("skills.allSuggestedAdded")}</p>
          ) : (
