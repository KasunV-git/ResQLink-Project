  ...
  return (
    <div className="w-full flex flex-col gap-4 md:gap-6" data-name="ProfilePage">
      ...
      <div className="anim-fade-in-up flex flex-col gap-1">
        <h1 className="font-semibold text-3xl text-slate-900 tracking-tight">{t("profile.title")}</h1>
        <p className="text-slate-500 text-base">{t("profile.subtitle")}</p>
      </div>
      ...
      <div className="anim-fade-in-up d-100 bg-white border border-slate-200 rounded-xl p-4 md:p-6 shadow-sm flex flex-col gap-5 md:gap-6">
        <div className="flex justify-between items-center pb-3 border-b border-slate-100">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="font-semibold text-base text-slate-900">{t("profile.profileInfo")}</h2>
          </div>
          {!isEditing && (
            <button
        ...
        <form onSubmit={handleSave} className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t("profile.firstName")}</label>
              <input
                type="text"
                value={firstName}
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t("profile.lastName")}</label>
              <input
                type="text"
                value={lastName}
          ...
          {/* Email Address */}
          <div className="flex flex-col gap-2">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t("profile.emailAddress")}</label>
            ...
            <span className="text-slate-400 text-xs pl-0.5">{t("profile.emailCannotChange")}</span>
          </div>

          {/* Mobile Number */}
          <div className="flex flex-col gap-2">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t("profile.mobileNumber")}</label>
            <div className="relative">
          ...
          {/* Account Role */}
          <div className="flex flex-col gap-2">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t("profile.accountRole")}</label>
            <div className="flex items-center gap-2 border border-emerald-100 bg-emerald-50/30 rounded-lg p-2.5 w-fit">
              <Shield className="w-4 h-4 text-emerald-600" />
              <span className="text-emerald-700 text-sm font-semibold">{user?.role || t("auth.volunteerRole")}</span>
            </div>
          </div>
      ...
      <div className="anim-fade-in-up d-200 bg-white border border-red-200 rounded-xl p-4 md:p-6 shadow-sm flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          <h3 className="font-semibold text-base text-slate-900">{t("profile.signOut")}</h3>
          <p className="text-slate-500 text-sm">{t("profile.signOutDesc")}</p>
        </div>
        <button
          onClick={onLogout}
